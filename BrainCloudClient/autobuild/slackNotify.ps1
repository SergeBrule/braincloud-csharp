...
Function RunRegexp {
 ...
 if($buildParams['cmakeFailed']) {
    ...
    ReportFailure "CMake Generation Failed"
    exit 1
  }

  if($buildParams['buildFailed']) {
    LogErrorToConsole('CMake Build Failed')
    ReportFailure "CCMake Build Failed"
    exit 1
  }
  ...
  $TestResults=$TestsDidFail -split 'Test Failure : '

  $NotificationJSON=BuildMessageJSON($TestResults)
